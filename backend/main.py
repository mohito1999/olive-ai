...
DEFAULT_INITIAL_MESSAGE = "Hello, am I speaking to {name}?"
DEFAULT_PROMPT = """You can Kunal from {company}. You are free to call the tools provided to you. NEVER say anything else when calling a tool, not even things like 'Please call the following tool'. You can talk in English or Hindi. 
...
Start by Introducing yourself as Kunal and say you are calling from {company}. Verify that you are speaking with the customer by using their {name} wherever applicable. Then you Identify the Issue and Inform the customer that you noticed they did not go through with the process for purchasing {company_product}. Ask if there is any assistance needed and wait for their response.
...
class OutboundCallRequest(BaseModel):
    ...
    interrupt_sensitivity: Optional[InterruptSensitivity] = None
...
async def api_start_outbound_call(
    ...
    initial_message = req.initial_message or DEFAULT_INITIAL_MESSAGE
    initial_message = initial_message.format(**prompt_variables)
    ...
    synth_config = GoogleSynthesizerConfig.from_telephone_output_device(
        language_code="hi-IN", voice_name=voice, pitch=-10.0
    )
